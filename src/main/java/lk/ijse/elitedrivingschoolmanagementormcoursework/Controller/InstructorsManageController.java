...
public class InstructorsManageController implements Initializable {
    ...
    public void btnDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (result.isPresent() && result.get() == ButtonType.YES) {
            InstructorTM selectedItem = (InstructorTM) tblInstructors.getSelectionModel().getSelectedItem();
    ...
    private void loadAllInstructors() {
        try {
            tblInstructors.setItems(FXCollections.observableArrayList(
                    instructorsBO.getAllInstructors().stream().map(instructor -> new InstructorTM(
